...
- **Design tokens & UI:** `docs/DESIGN.md` là nguồn sự thật duy nhất cho color/typography/radius/spacing/components (chuẩn Google Labs `design.md` spec, alpha). Mọi sửa view, Blade, Livewire, Alpine phải map token → Tailwind class theo file này; cần mở rộng → đề xuất qua PR riêng cho `docs/DESIGN.md`.
- **Merge PR (khi Sếp yêu cầu):** dùng `gh pr merge <N> --repo diepxuan/portal --squash --delete-branch` (hoặc `--merge`/`--rebase` tuỳ convention). KHÔNG dùng `git merge --squash` local + `gh pr close` thay thế — cách này mất PR state `MERGED`, mất attribution cho contributor, mất audit trail. Verify sau merge: `state == MERGED`, `mergeCommit.oid` không rỗng. Xem `AGENTS.md` §5.1.
...
- **Tuân thủ `docs/DESIGN.md`** khi đụng UI/UX — map design token sang Tailwind utility class; không tự ý tạo class mới ngoài palette/spacing đã định nghĩa.
- **Merge PR = `gh pr merge`** — không dùng `git merge` local + `gh pr close`. Verify sau merge: `state == MERGED`, `mergeCommit.oid` không rỗng.
