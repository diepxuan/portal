...
- **Module**: CO (Cost)
- **Loại**: Report
- **Chức năng**: Báo cáo giá thành 03 - Chi tiết chi phí theo tài khoản và sản phẩm chi tiết (phiên bản khác)

## Forms

### frmCORptGt03.cs
- **Kế thừa**: frmReport
- **Chức năng**: Form báo cáo giá thành chi tiết (variant)
...
## So sánh với CORptGt02

| Đặc điểm | CORptGt02 | CORptGt03 |
|----------|-----------|-----------|
| Kế thừa | frmReport | frmReport |
| Controls | Giống nhau | Giống nhau |
| Logic | Giống nhau | Giống nhau |
| Khác biệt | ClientSize = 564, 275 | ClientSize = 564, 261 |
| TabFilter Size | 556, 221 | 556, 207 |

Có thể CORptGt03 là phiên bản rút gọn hoặc điều chỉnh kích thước của CORptGt02.

## Mapping sang PHP

### Controller
```php
class CORptGt03Controller extends ReportController {
    public function index() {
        return view('co.reports.gt03');
    }
    ...
    public function getData(Request $request) {
        ...
        $data = DB::select('EXEC sp_CORptGt03 ?, ?, ?, ?, ?', $params);
...
## Ghi chú
- CORptGt03 có cấu trúc và logic giống hệt CORptGt02
- Khác biệt duy nhất là kích thước form nhỏ hơn một chút
- Có thể là phiên bản dành cho màn hình nhỏ hoặc layout khác
- Cùng chức năng: báo cáo chi tiết chi phí theo tài khoản và SPCT
