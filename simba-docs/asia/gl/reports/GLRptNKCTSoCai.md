...
- **Loại**: Report
- **Chức năng**: Báo cáo nhật ký chung tổng hợp sổ cái

## Forms

### frmGLRptNKCTSoCai.cs
- **Kế thừa**: frmReport
- **Chức năng**: Form báo cáo nhật ký chung tổng hợp sổ cái theo tài khoản
...
|----|---------|--------------|--------|
| SP từ ReportInfo | Lấy dữ liệu NKC Sổ cái | @pMa_cty, @pNgay1, @pNgay2, @pTk | DataTable |
...
- Text của form là "frmGLRptNKC08" (có thể là copy từ NKC08)
...
### Controller
```php
class GLRptNKCTSoCaiController extends ReportController {
    public function index() {
        $kyBaoCaoOptions = $this->getKyBaoCaoOptions();
        $mauBaoCaoOptions = $this->getReportTemplates('GLRptNKCTSoCai');
        return view('gl.rpt_nkc_tsc', compact('kyBaoCaoOptions', 'mauBaoCaoOptions'));
    }
    ...
    public function generate(Request $request) {
        ...
        $data = $this->reportService->getNkcTscData($params);
...
### Repository
```php
class GLRptNKCTSoCaiRepository {
    public function getData($params) {
        return DB::select('EXEC [sp_GetNkcTscData] ?, ?, ?, ?', [
            $params['ma_cty'],
            $params['ngay1'],
            $params['ngay2'],
            $params['tk']
...
## Ghi chú
- Đây là báo cáo nhật ký chung tổng hợp sổ cái
- Giao diện và cấu trúc tương tự NKCTNK1
- Text của form vẫn là "frmGLRptNKC08" (có thể là lỗi copy/paste)
- Báo cáo này tập trung vào sổ cái (ledger) thay vì nhật ký chung chi tiết
