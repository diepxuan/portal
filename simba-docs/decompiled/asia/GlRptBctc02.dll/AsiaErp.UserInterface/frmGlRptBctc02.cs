...
[DesignerGenerated]
public class frmGlRptBctc02 : frmReport
{
	...
	public frmGlRptBctc02(string menuid)
		: base(menuid)
	{
		__ENCAddToList(this);
		sMenuSubTitle = ". F5 - Chi tiet cong thuc/So chu T, Ctrl+F5 - So chi tiet, Shift+F5 - Can doi PS cong no,  F7 - Xem, F9 - In";
		InitializeComponent();
	}
	...
	private void InitializeComponent()
	{
		...
		CheckBox obj14 = chkChiTieuCoSoLieu;
	...
	protected override void InitOtherComponents()
	{
		...
		sMenuSubTitle = AsiaErp.Framework.Environment.getLangSysMessageResX("#GLRPTBCTC02_MENUSUBTITLE").value;
	...
	private void AsTabControl_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		{
			((Form)this).Text = base.MyMenuInfo.bar.Trim() + sMenuSubTitle;
		}
	}
	...
	protected override bool LoadData()
	{
		...
		{
			DataRow[] array = data.Select("ma_so='270'");
			if ((array != null && array.Length > 0) ? true : false)
			{
				DataRow dataRow = array[0];
				num = Conversions.ToDouble(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(dataRow["tien0"]), isNum: true));
				num2 = Conversions.ToDouble(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(dataRow["tien"]), isNum: true));
			}
			array = data.Select("ma_so='440'");
			if ((array != null && array.Length > 0) ? true : false)
			{
				DataRow dataRow = array[0];
				num3 = Conversions.ToDouble(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(dataRow["tien0"]), isNum: true));
				num4 = Conversions.ToDouble(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(dataRow["tien"]), isNum: true));
			}
			if (num != num3)
			{
				text = string.Format(Helper.GetMessContent(50203), Strings.Format((object)(num - num3), "Standard"));
			}
			if (num2 != num4)
			{
				text += "\r\n";
				text += string.Format(Helper.GetMessContent(50204), Strings.Format((object)(num2 - num4), "Standard"));
			}
