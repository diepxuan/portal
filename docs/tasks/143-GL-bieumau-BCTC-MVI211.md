# Task 143: GLMAUBCTCTMVI211

## Nhom: GL (General Ledger - So cai tong hop)

## Muc tieu
Chuyen doi chuc nang Mau bao cao tai chinh I211 tu .NET sang PHP Laravel, dat ket qua tuong duong Phieubaono.

## Chi tiet
- **DLL:** GLMAUBCTCTMVI211.dll
- **Chuc nang:** Quan ly mau bao cao tai chinh I211
- **Loai:** Bieu mau (MAU)
...
- **Template:** BCTC I211 (Thong tu 200/2014/TT-BTC)

## Tai lieu tham khao
`/root/.openclaw/workspace/projects/SimbaSql/docs/decompiled/asia/GLMAUBCTCTMVI211.dll/README.md`

---

## Cau truc du lieu

### Bang: MAUBCTCI211 (Mau bao cao tai chinh I211)
...
| cong_thuc | nvarchar | 200 | Cong thuc tinh toan |
...
### 1. frmGLMAUBCTCTMVI211 (Form xem danh sach)
- **Ke thua:** frmDMComplexView
- **Chuc nang:** Hien thi danh sach chi tieu I211, tim kiem, phan trang
- **Controls:**
...
### 2. frmGLMAUBCTCTMVI211Edit (Form them/sua)
- **Ke thua:** frmDMComplexEdit
- **Chuc nang:** Nhap, sua thong tin chi tieu I211
- **Controls:**
...
| txtCong_thuc | TextBox | cong_thuc | Cong thuc tinh toan |
...
| SP Name | Mo ta |
|---------|-------|
| SP_GL_MAU_BCTC_I211_GET | Lay danh sach chi tieu I211 |
| SP_GL_MAU_BCTC_I211_GETBYID | Lay chi tiet 1 chi tieu |
| SP_GL_MAU_BCTC_I211_INSERT | Them chi tieu moi |
| SP_GL_MAU_BCTC_I211_UPDATE | Cap nhat chi tieu |
| SP_GL_MAU_BCTC_I211_DELETE | Xoa chi tieu |
| SP_GL_MAU_BCTC_I211_FIND | Tim kiem chi tieu |
| SP_GL_MAU_BCTC_I211_CHECK | Kiem tra trung ma |

### SP_GET (reference)

```sql
-- Lay danh sach chi tieu I211
EXEC SP_GL_MAU_BCTC_I211_GET
...
```sql
-- Them chi tieu moi
EXEC SP_GL_MAU_BCTC_I211_INSERT
    @pMa_cty VARCHAR(50),
    @pMau VARCHAR(50),
    @pMa_so VARCHAR(50),
    @pChi_tieu NVARCHAR(200),
    @pNd_chtieu NVARCHAR(500) = NULL,
    @pCong_thuc NVARCHAR(200) = NULL,
...
```php
// app/Models/GL/MAUBCTCI211.php
namespace Diepxuan\Simba\Models\GL;

class MAUBCTCI211 extends Model
{
    protected $table = 'MAUBCTC_I211';
...
```php
// diepxuan/laravel-catalog/src/Http/Livewire/GL/Bieumau/BCTCVI211.php
namespace Diepxuan\Catalog\Http\Livewire\GL\Bieumau;

class BCTCVI211 extends Component
{
    const MA_CT = 'GL';
    ...
    {
        return view('catalog::gl.bieumau.bctcvi211');
    }
}
```
...
```php
// diepxuan/laravel-catalog/src/Http/Livewire/GL/Bieumau/BCTCVI211Edit.php
namespace Diepxuan\Catalog\Http\Livewire\GL\Bieumau;

class BCTCVI211Edit extends Component
{
    public ?string $pMau = null;
    public ?string $pMaSo = null;
    public string $pChiTieu = '';
    public string $pNdChtieu = '';
    public string $pCongThuc = '';
    ...
    public function render(): View
    {
        return view('catalog::gl.bieumau.bctcvi211-edit');
    }
}
```
...
├── bctcvi211.blade.php         (List page)
├── bctcvi211-edit.blade.php    (Modal edit)
```
...
Route::prefix('catalog/gl/bieumau')
    ...
    ->group(function () {
        Route::get('/bctcvi211', [BCTCVI211::class, 'render'])->name('bctcvi211');
    });
```
...
|------|---------|------|---------|
| Model | laravel-simba | MAUBCTCI211.php | Chinh |
| Model | laravel-simba | DMTAIKHOAN.php | Lien quan (FK) |
| Component | laravel-catalog | BCTCVI211.php | List |
| Component | laravel-catalog | BCTCVI211Edit.php | Edit |
...
- [ ] Tao Model MAUBCTCI211
- [ ] Tao Livewire BCTCVI211 (list)
- [ ] Tao Livewire BCTCVI211Edit (modal)
- [ ] Tao Views (list + edit)
- [ ] Them Routes
...
- **Route:** `gl.task.143` hoặc GL report dynamic registry nếu có metadata.
