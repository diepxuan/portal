# Task 151: GLMAUBCTCTMI

## Nhom: GL (General Ledger - So cai tong hop)

## Muc tieu
Chuyen doi chuc nang Mau bao cao tai chinh I tu .NET sang PHP Laravel, dat ket qua tuong duong Phieubaono.

## Chi tiet
- **DLL:** GLMAUBCTCTMI.dll
- **Chuc nang:** Quan ly mau bao cao tai chinh I
- **Loai:** Bieu mau (MAU)
- **Assembly Title:** GLBCTC071234
- **Version:** 1.0.0.0
- **Namespace:** `AsiaErp.UserInterface`
- **Company:** Asia Software Development JSC
- **Ma_ct:** GL (General Ledger)
- **Template:** BCTC I (Thong tu 200/2014/TT-BTC)

## Tai lieu tham khao
`/root/.openclaw/workspace/projects/SimbaSql/docs/decompiled/asia/GLMAUBCTCTMI.dll/README.md`

---

## Cau truc du lieu

### Bang: MAUBCTCBMI (Mau bao cao tai chinh I)

| Truong | Kieu | Chieu dai | Mo ta |
|--------|------|-----------|-------|
| mau | varchar | 50 | Ma mau QD (khoa chinh) |
| ma_so | varchar | 50 | Ma chi tieu |
| chi_tieu | nvarchar | 200 | Ten chi tieu |
| nd_chtieu | nvarchar | 500 | Noi dung chi tieu |
| IsPrint | bit | 1 | Co in hay khong |
| IsItalic | bit | 1 | In nghieng |
| bold | bit | 1 | In dam |

### Table Navigation

| Bang | Mo ta | Relationship |
|------|-------|---------------|
| GLRpt* | Cac bao cao | Lookup mau |

---

## Form classes

### 1. frmGLMAUBCTCTMI (Form xem danh sach)
- **Ke thua:** frmDMComplexView
- **Chuc nang:** Hien thi danh sach chi tieu BCTC I, tim kiem, phan trang
- **Controls:**

| Control | Type | DataField | Mo ta |
|---------|------|-----------|-------|
| dgvDM | DataGridView | - | Luoi hien thi danh sach |
| txtSearch | TextBox | - | Tim kiem theo ma_so/chi_tieu |
| btnAdd | Button | - | Nut them moi |
| btnEdit | Button | - | Nut sua |
| btnDelete | Button | - | Nut xoa |

### 2. frmGLMAUBCTCTMIEdit (Form them/sua)
- **Ke thua:** frmDMComplexEdit
- **Chuc nang:** Nhap, sua thong tin chi tieu BCTC I
- **Controls:**

| Control | Type | DataField | Mo ta |
|---------|------|-----------|-------|
| txtMa_chtieu | AsTextBox | ma_so | Ma chi tieu (AutoComplete, Uppercase) |
| txtChi_tieu | TextBox | chi_tieu | Ten chi tieu |
| txtNd_chtieu | TextBox | nd_chtieu | Noi dung chi tieu (Multiline) |
| txtMauQD | AsTextBox | mau | Mau quyet dinh |
| ChkIs_print | CheckBox | IsPrint | Co in |
| ChkIs_italic | CheckBox | IsItalic | In nghieng |
| chkKsd | AsCheckBox | bold | In dam |

---

## Stored Procedures

| SP Name | Mo ta |
|---------|-------|
| SP_GL_MAU_BCTC_I_GET | Lay danh sach chi tieu BCTC I |
| SP_GL_MAU_BCTC_I_GETBYID | Lay chi tiet 1 chi tieu |
| SP_GL_MAU_BCTC_I_INSERT | Them chi tieu moi |
| SP_GL_MAU_BCTC_I_UPDATE | Cap nhat chi tieu |
| SP_GL_MAU_BCTC_I_DELETE | Xoa chi tieu |
| SP_GL_MAU_BCTC_I_FIND | Tim kiem chi tieu |
| SP_GL_MAU_BCTC_I_CHECK | Kiem tra trung ma |

### SP_INSERT (reference)

```sql
-- Them chi tieu moi
EXEC SP_GL_MAU_BCTC_I_INSERT
    @pMa_cty VARCHAR(50),
    @pMau VARCHAR(50),
    @pMa_so VARCHAR(50),
    @pChi_tieu NVARCHAR(200),
    @pNd_chtieu NVARCHAR(500) = NULL,
    @pIsPrint BIT = 1,
    @pIsItalic BIT = 0,
    @pBold BIT = 0,
    @pStatus INT OUTPUT
```

---

## Business Logic

### Validate Rules

| Rule | Field | Message |
|------|-------|---------|
| Required | ma_so | 'Ma chi tieu khong duoc trong' |
| Max length 50 | ma_so | 'Ma chi tieu toi da 50 ky tu' |
| Unique | ma_so + mau | 'Ma chi tieu da ton tai trong mau nay' |
| Required | chi_tieu | 'Chi tieu khong duoc trong' |
| Max length 200 | chi_tieu | 'Chi tieu toi da 200 ky tu' |

### Business Rules

1. **Ma chi tieu**:
   - Uppercase tu dong
   - AutoComplete tu danh sach
   - Khong cho phep trung trong cung mau

2. **Mau quyet dinh**:
   - Mac dinh lay tu CompanyInformations.Qd_cdkt

3. **Tuy chon in**:
   - IsPrint: Mac dinh = true khi them moi
   - IsItalic: Mac dinh = false
   - Bold: Mac dinh = false

---

## Mapping PHP

### 1. Model

```php
// app/Models/GL/MAUBCTCBMI.php
namespace Diepxuan\Simba\Models\GL;

class MAUBCTCBMI extends Model
{
    protected $table = 'MAUBCTC_BMI';
    protected $primaryKey = ['mau', 'ma_so'];
    protected $keyType = 'string';
    public $incrementing = false;
    protected $connection = 'simba';

    protected $fillable = [
        'mau',
        'ma_so',
        'chi_tieu',
        'nd_chtieu',
        'IsPrint',
        'IsItalic',
        'bold',
    ];

    protected $casts = [
        'IsPrint' => 'boolean',
        'IsItalic' => 'boolean',
        'bold' => 'boolean',
    ];
}
```

### 2. Livewire Component (Edit)

```php
// diepxuan/laravel-catalog/src/Http/Livewire/GL/Bieumau/BCTCBMIEdit.php
namespace Diepxuan\Catalog\Http\Livewire\GL\Bieumau;

class BCTCBMIEdit extends Component
{
    public ?string $pMau = null;
    public ?string $pMaSo = null;
    public string $pChiTieu = '';
    public string $pNdChtieu = '';
    public bool $pIsPrint = true;
    public bool $pIsItalic = false;
    public bool $pBold = false;

    protected $rules = [
        'pMaSo' => 'required|string|max:50',
        'pChiTieu' => 'required|string|max:200',
    ];

    public function render(): View
    {
        return view('catalog::gl.bieumau.bctcbi-edit');
    }
}
```

### 3. Views

```
resources/views/catalog/gl/bieumau/
├── bctcbi.blade.php         (List page)
├── bctcbi-edit.blade.php    (Modal edit)
```

### 4. Routes

```php
Route::prefix('catalog/gl/bieumau')
    ->name('catalog.gl.bieumau.')
    ->group(function () {
        Route::get('/bctcbi', [BCTCBMI::class, 'render'])->name('bctcbi');
    });
```

---

## Dependencies

| Loai | Package | File | Ghi chu |
|------|---------|------|---------|
| Model | laravel-simba | MAUBCTCBMI.php | Chinh |
| Component | laravel-catalog | BCTCBMI.php | List |
| Component | laravel-catalog | BCTCBMIEdit.php | Edit |

---

## Progress Checklist

- [ ] Phan tich yeu cau & review task nafy
- [ ] Tao Stored Procedure classes
- [ ] Tao Model MAUBCTCBMI
- [ ] Tao Livewire BCTCBMI (list)
- [ ] Tao Livewire BCTCBMIEdit (modal)
- [ ] Tao Views (list + edit)
- [ ] Them Routes
- [ ] Test CRUD operations
- [ ] Test Integration voi GLRpt*

---

## Ghi chu

- BCTC I la mau bao cao don gian, khong co tai khoan
- Theo Thong tu 200/2014/TT-BTC
---
## Audit Status
- **Ngày audit:** 2026-05-10
- **Kết quả:** PENDING — spec đầy đủ, chưa implement

## Portal implementation status

- **Status:** DONE (GL route/report shell)
- **Route:** `gl.task.151` hoặc GL report dynamic registry nếu có metadata.
- **Note:** Shell/read-only; process/write không execute khi chưa audit payload.
