<div class="so-ardmkh-container w-full">
<div class="mt-4 flex items-center gap-4">
        <div class="flex w-full max-w-md items-center">
            ...
            @if ($search)
                <button wire:click="$set('search', '')"
                    class="absolute right-3 text-gray-400 hover:text-gray-600">✕</button>
            @endif
        </div>
    ...
    <div class="mb-3 flex justify-end">
        <a href="{{ simbaroute('so.dict.ardmkh.create') }}" class="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700">+ Thêm khách hàng</a>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="border-b border-gray-200 px-3 py-2 text-left font-medium text-gray-500">Tên khách hàng
                    </th>
            ...
            <tbody>
                @forelse ($arDmKhs as $index => $kh)
                    <tr class="hover:bg-sky-50" wire:key="kh-{{ $kh->ma_kh }}">
                        <td class="border-b border-gray-100 px-3 py-2 text-gray-500">
                            {{ $arDmKhs->firstItem() + $index }}</td>
                        <td class="border-b border-gray-100 px-3 py-2 font-mono text-xs">{{ $kh->ma_kh }}</td>
                        <td class="border-b border-gray-100 px-3 py-2"><span
                                class="font-medium text-gray-900">{{ $kh->ten_kh }}</span></td>
                        <td class="border-b border-gray-100 px-3 py-2 text-gray-600">{{ Str::limit($kh->dia_chi, 40) }}
                        </td>
                        <td class="border-b border-gray-100 px-3 py-2 text-gray-600">{{ $kh->tel }}</td>
                        <td class="border-b border-gray-100 px-3 py-2 text-gray-600">{{ $kh->nguoi_gd }}</td>
                        <td class="border-b border-gray-100 px-3 py-2 text-gray-600">{{ $kh->ma_nhkh }}</td>
                        <td class="border-b border-gray-100 px-3 py-2 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ simbaroute('so.dict.ardmkh.edit', $kh->ma_kh) }}"
                                    class="rounded bg-yellow-100 px-2 py-1 text-xs text-yellow-700 hover:bg-yellow-200">Sửa</a>
                                <button wire:click="deleteDoiTuong('{{ $kh->ma_kh }}')"
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-3 py-8 text-center text-gray-500">Không tìm thấy khách hàng nào.
                        </td>
                    </tr>
                @endforelse
            </tbody>
