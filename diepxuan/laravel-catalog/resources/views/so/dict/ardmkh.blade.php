<div class="so-ardmkh-container w-full">
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500">ARDMKH - SO/AR customer context</p>
            </div>
            <a
    ...
    <div class="mt-4 flex items-center gap-4">
        <div class="flex w-full max-w-md items-center">
            ...
            />
            @if ($search)
                <button wire:click="$set('search', '')" class="absolute right-3 text-gray-400 hover:text-gray-600">✕</button>
            @endif
        </div>
            ...
            <tbody>
                @forelse ($arDmKhs as $index => $kh)
                    <tr class="hover:bg-sky-50" wire:key="kh-{{ $kh->ma_kh }}">
                        ...
                        <td class="border-b border-gray-100 px-3 py-2"><span class="font-medium text-gray-900">{{ $kh->ten_kh }}</span></td>
                        <td class="border-b border-gray-100 px-3 py-2 text-gray-600">{{ Str::limit($kh->dia_chi, 40) }}</td>
                        ...
                        <td class="border-b border-gray-100 px-3 py-2 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ simbaroute('so.dict.ardmkh.edit', $kh->ma_kh) }}" class="rounded bg-yellow-100 px-2 py-1 text-xs text-yellow-700 hover:bg-yellow-200">Sửa</a>
                                <button wire:click="deleteDoiTuong('{{ $kh->ma_kh }}')" wire:confirm="Bạn có chắc chắn muốn xóa khách hàng {{ $kh->ma_kh }}?" class="rounded bg-red-100 px-2 py-1 text-xs text-red-700 hover:bg-red-200">Xóa</button>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="px-3 py-8 text-center text-gray-500">Không tìm thấy khách hàng nào.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
