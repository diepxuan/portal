...
<div class="relative" x-data="tkInputComponent(@js($glDmTks))" x-init="initComponent()" @click.outside="showDropdown = false"
    @keydown.escape.window="showDropdown = false">

    <!-- Input field -->
    <div class="relative">
        <input {{ $disabled ? 'disabled' : '' }} type="text" placeholder="{{ $placeholder }}" x-model="search"
            @focus="showDropdown = true" @click="showDropdown = true" wire:model.live.debounce.300ms="pTk"
            class="{{ $class }} block w-full rounded-md border-gray-300 py-0 pr-8 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
