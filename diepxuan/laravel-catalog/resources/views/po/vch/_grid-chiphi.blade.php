<div class="space-y-2">
    <table class="min-w-max w-full text-xs">
        <thead class="bg-gray-50 border-b">
            <tr>
                <th class="px-2 py-1 text-left font-medium text-gray-600">Ma chi phi</th>
                <th class="px-2 py-1 text-left font-medium text-gray-600">Ten chi phi</th>
                <th class="px-2 py-1 text-center font-medium text-gray-600">PP</th>
                <th class="px-2 py-1 text-right font-medium text-gray-600">Tien CP NT</th>
                <th class="px-2 py-1 text-right font-medium text-gray-600">% VAT</th>
                <th class="px-2 py-1 text-right font-medium text-gray-600">Thue NT</th>
                <th class="px-2 py-1 text-right font-medium text-gray-600">Tong NT</th>
                <th class="px-2 py-1 text-left font-medium text-gray-600">Ma BP</th>
                <th class="px-2 py-1 text-left font-medium text-gray-600">Ma phi</th>
                <th class="px-2 py-1 text-left font-medium text-gray-600">Ma SPCT</th>
                <th class="px-2 py-1 text-left font-medium text-gray-600">Ma lo</th>
                <th class="px-2 py-1 text-center font-medium text-gray-600">Thao tac</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($pChiPhi ?? [] as $i => $row)
                <tr wire:key="cp-{{ $i }}" class="border-b">
                    <td class="px-1 py-1"><input wire:model="pChiPhi.{{ $i }}.ma_cp" class="w-24 rounded border border-gray-200 px-2 py-1 text-xs" placeholder="Ma CP" /></td>
                    ...
                    <td class="px-1 py-1 text-center">
                        <button type="button" wire:click="removeChiPhiRow({{ $i }})" class="rounded border border-red-200 px-2 py-1 text-xs text-red-600 hover:bg-red-50">Xoa</button>
                    </td>
                </tr>
            @empty
                <tr><td colspan="12" class="px-2 py-3 text-center text-xs text-gray-500">Chua co dong chi phi</td></tr>
            @endforelse
        </tbody>
    </table>
    <div class="flex justify-end gap-2 pt-2">
        <button type="button" wire:click="addChiPhiRow" class="rounded border border-gray-300 px-3 py-1 text-xs text-gray-700 hover:bg-gray-50">+ Them dong chi phi</button>
    </div>
</div>
