                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach ($phieuRows as $phieu)
                                        <tr wire:click="selectPhieu({{ $loop->index }})"
                                            ...
                                            <td class="whitespace-nowrap px-2 py-2 text-gray-700">{{ !empty($phieu['ngay_ct']) ? \Illuminate\Support\Carbon::parse($phieu['ngay_ct'])->format('d/m/Y') : '' }}</td>
                                            <td class="whitespace-nowrap px-2 py-2 font-mono text-gray-700">{{ $phieu['so_hd'] ?? '' }}</td>
                                            <td class="whitespace-nowrap px-2 py-2 text-gray-700">{{ !empty($phieu['ngay_hd']) ? \Illuminate\Support\Carbon::parse($phieu['ngay_hd'])->format('d/m/Y') : '' }}</td>
                            ...
                            <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                                <div class="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-3 py-2">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <a href="{{ simbaroute('po.vch.povchpo3.edit', $selectedPhieu['stt_rec']) }}"
                                            ...
                                            <tbody class="divide-y divide-gray-100">
                                                @foreach ($chiTietFiltered as $detail)
                                                    <tr class="hover:bg-sky-50">
                                                        ...
                                                        <td class="whitespace-nowrap px-2 py-2 text-right tabular-nums text-gray-700">{{ number_format((float) ($detail['gia_nt0'] ?? 0), 2, ',', '.') }}</td>
                                                        <td class="whitespace-nowrap px-2 py-2 text-right tabular-nums text-gray-700">{{ number_format((float) ($detail['tien_nt0'] ?? 0), 2, ',', '.') }}</td>
