        ...
        <tbody>
            @forelse ($pChiTiet ?? [] as $i => $row)
                <tr wire:key="ct-{{ $i }}" class="border-b">
                    ...
                    <td class="p-0 align-middle"><input type="number" step="0.01" wire:model.blur="pChiTiet.{{ $i }}.gia_nt0" wire:change="calculateChiTietRow({{ $i }})" class="w-28 rounded border border-gray-200 px-2 py-1 text-right text-xs" /></td>
                    <td class="p-0 align-middle"><input wire:model="pChiTiet.{{ $i }}.tien_nt0" class="w-28 rounded border border-gray-200 bg-gray-50 px-2 py-1 text-right text-xs" readonly /></td>
                    ...
                    <td class="p-0 align-middle"><input wire:model="pChiTiet.{{ $i }}.ck_nt" class="w-24 rounded border border-gray-200 bg-gray-50 px-2 py-1 text-right text-xs" readonly /></td>
                    <td class="p-0 align-middle"><input type="number" step="0.01" wire:model.blur="pChiTiet.{{ $i }}.tien_cp_nt" wire:change="calculateChiTietRow({{ $i }})" class="w-24 rounded border border-gray-200 px-2 py-1 text-right text-xs" /></td>
