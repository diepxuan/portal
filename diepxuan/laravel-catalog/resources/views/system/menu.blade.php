<div class="space-y-6" x-data="{
    ...
    <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        <div class="min-h-[200px] p-2"
                    ...
                    @if ($this->isVisible($node->id))
                        ...
                        @if (!$isDragging)
                            <div class="my-1"
                                ...
                                :class="{
                                    'border-t-2 border-blue-400': $wire.dropTargetId === {{ $nodeId }} && $wire
                                        .dropPosition === 'before'
                                }">
                            </div>
                        @endif
                        ...
                        <div class="{{ $isRecentlyUpdated ? 'border-green-300 bg-green-50' : 'border-gray-200' }} {{ $isDragging ? 'opacity-50' : '' }} {{ $isDropTarget && $dropPosition === 'inside' ? 'border-2 border-dashed border-blue-400 bg-blue-100' : '' }} group relative flex items-center rounded-lg border transition-all duration-200 hover:border-gray-300 hover:bg-gray-50"
                            ...
                            @if ($hasChildren)
                                <button type="button"
                                    class="{{ $isDropTarget && $dropPosition === 'inside' ? 'text-blue-500' : '' }} ml-2 flex h-6 w-6 items-center justify-center text-gray-500 hover:text-gray-700"
                            ...
                            <div class="flex flex-1 items-center px-3 py-2">
                                ...
                                <div class="col-span-1 w-16 text-center">
                                    <span class="rounded-full bg-gray-100 px-2 py-1 font-mono text-xs text-gray-700">
                                        {{ $nodeId }}
                                    </span>
                                </div>
                                ...
                                <div class="col-span-1 w-24 text-center">
                                    <div class="flex items-center justify-center space-x-1">
                                        ...
                                        <span class="min-w-[20px] text-sm font-medium text-gray-700">
                                            {{ $node->order }}
                                        </span>
                                        ...
                                        <button type="button"
                                            ...
                                            title="Di chuyển xuống" {{ !$canMoveDown ? 'disabled' : '' }}
                                ...
                                <!-- Actions -->
                                <div
                                    class="ml-4 flex items-center space-x-1 opacity-0 transition-opacity group-hover:opacity-100">
                                    @if (!$isEditing)
                                        <button type="button"
                                    ...
                                    <button type="button"
                                        ...
                                        wire:click="deleteNode({{ $nodeId }})" title="Xóa"
                                        wire:loading.attr="disabled">
                                        @if ($isSavingNode)
                        ...
                        @if ($isExpanded && $this->draggingNodeId !== null)
                            <div class="ml-{{ $node->level * 6 + 8 }} my-1 rounded-lg border-2 border-dashed border-blue-300 bg-blue-50 p-2 transition-all duration-200"
                                ...
                                <div class="text-center text-sm text-blue-600">
                                    <svg class="mx-auto mb-1 h-5 w-5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="mt-2" @dragover="if ($wire.draggingNodeId) { $wire.setDropTarget(null, 'after'); }"
                @dragleave="if ($wire.dropTargetId === null) { $wire.clearDropTarget(); }"
                :class="{
                    'border-2 border-dashed border-blue-400 bg-blue-50': $wire.dropTargetId === null && $wire
                        .draggingNodeId
                }">
