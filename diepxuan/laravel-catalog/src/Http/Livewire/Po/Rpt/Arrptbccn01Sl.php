 ...
 *
 * @lastupdate 2026-07-14 19:04:39
 */
...
/**
 * AP - Sổ chi tiết công nợ một nhà cung cấp - có số lượng.
 *
 * Mapping:
 * - menuID  : `10.30.23` (sysMenu.moduleid = PO; command = frmARRptBCCN01).
 * - DLL     : `ARRptBCCN01.dll` (form goc chia se giua AR `06.30.14` va AP `10.30.11`/`10.30.23`).
 * - SP      : `asARRptBCCN01SL` (report `ARBCCN012.rpt`).
 * - Route   : `_simba-source/po/rpt/arrptbccn01103023` (slug compact theo task 358).
 *
 * Trong AP view bind `ma_ncc` vao property `pMa_kh` (vi DLL goc khong phan biet AR/AP,
 * chi dung chung control `txtMa_Kh`); wrapper SP cung nhan field `ma_kh` cho ca AR va AP.
 */
class Arrptbccn01Sl extends Component
{
    ...
    public ?string $pTieu_de     = 'Sổ chi tiết công nợ một nhà cung cấp - có số lượng';
    ...
    {
        return view('catalog::po.rpt.arrptbccn01sl', [
            'rows'                => $this->rows,
            'presentationColumns' => $this->presentationColumns(),
        ]);
    }
    ...
    {
        // Quy uoc: dong chung tu (tong hop) khong co ma_vt;
        // dong san pham (chi tiet vat tu trong so co SL) co ma_vt.
        return '' !== trim((string) (data_get($row, 'ma_vt') ?? data_get($row, 'Ma_vt') ?? ''));
    }

    public function rowIsSummaryLine(array $row): bool
    {
        // Dong tong hop (DDK/TPS/DCK) khong co ma_vt va dien_giai la token Simba.
        // Kiem tra raw token truoc khi di qua reportLabel() (vi reportLabel dich token sang tieng Viet).
    ...
    /**
     * Static version of rowLineClass() used to build the dien_giai column
     * cellClass at column-render time (we do not have a $row context here).
     * Concrete per-row styling is applied inside the view via rowLineClass().
     */
    ...
    private static function csvDienGiaiWithIndent(array $row): string
    {
        ...
        // Prefix tab de Excel/CSV giu indent khi mo bang tinh.
        return "\t" . $value;
    }
