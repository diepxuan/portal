 ...
 * @coversNothing
 */
final class InVoucherRegistryTest extends TestCase
{
    public function testInVoucherRegistryMatchesSiDmCt(): void
    {
        $data     = new SimbaDocsDataRepository(\dirname(__DIR__, 5));
        $vouchers = $data->table('SiDmCt.md')->keyBy('ma_ct');

        foreach (InVoucherRegistry::vouchers() as $voucherCode => $metadata) {
    ...
    public function testInVoucherRegistryMatchesSysVoucherInfo(): void
    {
        $data     = new SimbaDocsDataRepository(\dirname(__DIR__, 5));
