 ...
 *
 * @lastupdate 2026-05-16 00:28:12
 */
 ...
 * @coversNothing
 */
final class FinanceVoucherRegistryTest extends TestCase
{
    public function testFinanceVoucherRegistryMatchesSiDmCt(): void
    {
        $data     = new SimbaDocsDataRepository(\dirname(__DIR__, 5));
        $vouchers = $data->table('SiDmCt.md')->keyBy('ma_ct');

        foreach (FinanceVoucherRegistry::vouchers() as $voucherCode => $metadata) {
    ...
    public function testFinanceVoucherRegistryMatchesSysVoucherInfo(): void
    {
        $data     = new SimbaDocsDataRepository(\dirname(__DIR__, 5));
