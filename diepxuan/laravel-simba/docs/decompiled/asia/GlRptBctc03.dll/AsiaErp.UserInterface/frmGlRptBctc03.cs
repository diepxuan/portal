...
[DesignerGenerated]
public class frmGlRptBctc03 : frmReport
{
	...
	public frmGlRptBctc03(string menuid)
		: base(menuid)
	{
		__ENCAddToList(this);
		InitializeComponent();
	...
	private void InitializeComponent()
	{
		...
		Point location = new Point(700, -532);
		((Control)obj2).Location = location;
		((Control)txtNgay2).TabIndex = 2;
		Button obj3 = cmdCancel;
		location = new Point(619, -532);
	...
	private void cboKyTruoc_SelectedValueChanged(object sender, EventArgs e)
	{
		if (cboKyTruoc.DataSource != null)
		{
			...
			{
				txtNgay01.Value = ((KyBaoCao)cboKyTruoc.SelectedItem).ngay1.AddYears(-1);
				txtNgay02.Value = ((KyBaoCao)cboKyTruoc.SelectedItem).ngay2.AddYears(-1);
	...
	protected override ArrayList CreateDrilldownArgument(ReportDrillDownInfo drilldownInfo, ref string sDrilldownMenuId, Keys keycode, Keys modifierKeys, ref string filterCondition4title, [Optional][DefaultParameterValue(null)] ref object datasource)
	{
		...
		else
		{
			...
			try
			{
				dataRowView = new DataView((DataTable)base.dgvBC.DataSource)[((DataGridViewBand)((DataGridView)base.dgvBC).CurrentRow).Index];
			}
			catch (Exception ex)
			{
				ProjectData.SetProjectError(ex);
