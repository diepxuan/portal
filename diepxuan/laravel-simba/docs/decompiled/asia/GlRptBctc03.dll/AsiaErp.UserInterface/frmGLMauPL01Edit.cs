...
public class frmGLMauPL01Edit : frmDMEdit
{
	...
	protected override bool ValidData()
	{
		...
		if ((int)checkState == 1)
		{
			...
			if (Conversions.ToBoolean((!Conversions.ToBoolean(chkCach_tinh.Checked) || !Conversions.ToBoolean((object)Operators.ConditionalCompareObjectNotEqual((object)num, Commons.GetDataTable("asGLGetDmTk", CompanyInformations.CompanyID).Compute("count(tk)", "tk in ('" + ((TextBox)txtDsTK_co).Text.Trim().Replace(",", "','") + "')"), false))) ? ((object)false) : ((object)true)) && !flag)
			{
				SetControlError((Control)(object)txtDsTk_no, Helper.GetMessContent(50091));
