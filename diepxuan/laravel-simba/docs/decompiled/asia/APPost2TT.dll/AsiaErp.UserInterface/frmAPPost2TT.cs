...
[DesignerGenerated]
public class frmAPPost2TT : frmAsiaRoot
{
	...
	protected frmAPPost2TT(string menuid)
		: base(menuid)
	{
	...
	private void InitializeComponent()
	{
		...
		((Control)this).Name = "frmAPPost2TT";
		...
		checked
		{
			...
			else
			{
				...
				for (int i = 0; i <= num; i++)
				{
					...
					int num2 = aPPost2TTDAO.Insert(arrayList.ToArray());
					if (num2 != 0)
					{
	...
	private void btnXoa_Click(object sender, EventArgs e)
	{
		...
		checked
		{
			...
			for (int i = 0; i <= num; i++)
			{
				...
				arrayList.Add(RuntimeHelpers.GetObjectValue(((DataGridView)adgvPhanbo_hd)[((DataGridViewColumn)dgvcTien_tt).Name, i].Value));
				APPost2TTDAO aPPost2TTDAO = (APPost2TTDAO)DAOFactory.CreateDAOInstance("APPost2TTDAO", "APPost2TT");
				int num2 = aPPost2TTDAO.Delete(arrayList.ToArray());
				if (num2 != 0)
				{
					CMessageBox.Show(num2);
					bTrangthai = true;
	...
	private void GetRow()
	{
		...
		arrayList.Add("");
		arrayList.Add(AsiaErp.Framework.Environment.GetPOConfiguration().tk_pt);
		arrayList.Add("");
		APPost2TTDAO aPPost2TTDAO = (APPost2TTDAO)DAOFactory.CreateDAOInstance("APPost2TTDAO", "APPost2TT");
	...
	private void MoveRow()
	{
		checked
		{
			if (bTrangthai && f_oDt.Rows.Count != 0)
			{
				...
				GetPH(bIns: false);
				APPost2TTDAO aPPost2TTDAO = (APPost2TTDAO)DAOFactory.CreateDAOInstance("APPost2TTDAO", "APPost2TT");
				DataTable data = aPPost2TTDAO.GetData(AsiaErp.Framework.Environment.GetPOConfiguration().tk_pt, Conversions.ToString(oRow["Ma_kh"]), Conversions.ToDate(oRow["Ngay_ct"]), "2", Conversions.ToString(oRow["Stt_rec"]));
	...
	protected void ShowFind(bool type)
	{
		...
		frmAPPost2TTFind frmAPPost2TTFind2 = new frmAPPost2TTFind();
		if (type)
		{
			frmAPPost2TTFind2.f_bIns = type;
		}
		else
		{
			frmAPPost2TTFind2.f_bIns = type;
		...
		((Form)frmAPPost2TTFind2).ShowDialog();
		if (frmAPPost2TTFind2.f_oDtct == null)
		{
			SetNormal();
			MoveRow();
		}
		else if (frmAPPost2TTFind2.f_oDtct.Rows.Count == 0)
		{
			SetNormal();
			MoveRow();
		}
		...
		{
			oRow = frmAPPost2TTFind2.f_oRow;
			GetPH(frmAPPost2TTFind2.f_bIns);
			((DataGridView)adgvPhanbo_hd).DataSource = frmAPPost2TTFind2.f_oDtct;
		}
	}
