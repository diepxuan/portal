...
[DesignerGenerated]
public class frmRestore : frmAsiaRoot
{
	...
	public frmRestore()
	{
		...
		((Form)this).Load += frmRestore_Load;
		__ENCAddToList(this);
		oRes = new Restore();
	...
	private void DoRestore()
	{
		...
		SqlConnection val2 = new SqlConnection(((DbConnectionStringBuilder)(object)val).ConnectionString);
		val2.Open();
		SqlCommand val3 = new SqlCommand("SELECT CONVERT(varchar(5), spid) as spid FROM master..sysprocesses WHERE dbid = db_id('" + DatabaseConfigurations.DATADatabaseName + "')", val2);
		SqlDataReader val4 = val3.ExecuteReader();
		string text = "";
		while (val4.Read())
		{
			text = Conversions.ToString(Operators.AddObject(Operators.AddObject((object)(text + "Kill "), val4["spid"]), (object)";"));
		}
		val4.Close();
		if (!text.Equals(string.Empty))
		{
			val3.CommandText = "Exec('" + text + "')";
			val3.ExecuteNonQuery();
		}
		ServerConnection val5 = new ServerConnection(val2);
		Server val6 = new Server(val5);
		oRes.ReplaceDatabase = true;
		oRes.Action = (RestoreActionType)0;
		((BackupRestoreBase)oRes).Devices.AddDevice(szPathName, (DeviceType)2);
		((BackupRestoreBase)oRes).Restart = true;
		((BackupRestoreBase)oRes).Database = DatabaseConfigurations.DATADatabaseName;
		((BackupRestoreBase)oRes).PercentCompleteNotification = 1;
		((BackupRestoreBase)oRes).ContinueAfterError = false;
		((BackupRestoreBase)oRes).Checksum = true;
		((BackupRestoreBase)oRes).Information += new ServerMessageEventHandler(OnInformation);
		((BackupRestoreBase)oRes).PercentComplete += new PercentCompleteEventHandler(OnPercentComplete);
		((BackupRestoreBase)oRes).Complete += new ServerMessageEventHandler(OnComplete);
		oRes.SqlRestoreAsync(val6);
	}
