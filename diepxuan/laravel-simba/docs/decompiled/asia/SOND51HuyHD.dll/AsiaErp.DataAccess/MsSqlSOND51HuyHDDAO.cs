...
public class MsSqlSOND51HuyHDDAO : MsSqlDAO, SOND51HuyHDDAO
{
	[DebuggerNonUserCode]
	public MsSqlSOND51HuyHDDAO()
	{
	}
	...
	public int Delete(string ma_tb)
	{
		...
		_command.Parameters.AddWithValue("@pKieu_xl", (object)"HUY");
	...
	int SOND51HuyHDDAO.Delete(string ma_tb)
	{
	...
	{
		_command.CommandType = CommandType.StoredProcedure;
		_command.CommandText = "asSOGetBKHDCT";
		_command.Parameters.Clear();
		_command.Parameters.AddWithValue("@pMa_cty", (object)CompanyInformations.CompanyID);
		return GetDataTable();
	}

	DataTable SOND51HuyHDDAO.GetData()
	{
	...
	DataTable SOND51HuyHDDAO.GetData(string ma_tb)
	{
		//ILSpy generated this explicit interface implementation from .override directive in GetData
	...
	int SOND51HuyHDDAO.Insert(string ma_tb, string ma_chung, string ten_loai, string mau_so, string so_seri, string tu_so, string den_so, string so_luong, string lien_hd, string ghi_chu)
	{
