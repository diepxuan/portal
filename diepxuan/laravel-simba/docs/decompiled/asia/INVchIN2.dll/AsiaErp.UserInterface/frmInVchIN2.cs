...
[DesignerGenerated]
public class frmInVchIN2 : frmVoucher
{
	...
	public frmInVchIN2(string menuid)
		: base(menuid)
	{
		__ENCAddToList(this);
		f_sGia = "";
	...
	public frmInVchIN2(string MenuID, string Stt_rec)
		: base(MenuID, Stt_rec)
	{
		__ENCAddToList(this);
		f_sGia = "";
	...
	private void InitializeComponent()
	{
		...
		Button obj4 = cmdNext;
		location = new Point(689, 428);
		((Control)obj4).Location = location;
		Button obj5 = cmdPrevious;
		location = new Point(664, 428);
		((Control)obj5).Location = location;
		Button obj6 = cmdFirst;
		location = new Point(639, 428);
		((Control)obj6).Location = location;
		((Control)lblSo_Ct).Anchor = (AnchorStyles)5;
		Label obj7 = lblSo_Ct;
		location = new Point(533, 44);
		...
		((Control)adgvDetail).Cursor = Cursors.Default;
		...
		((Control)txtT_So_luong).TabIndex = 258;
		...
		((Control)this).Name = "frmInVchIN2";
		((Form)this).Text = "frmInVchIN2";
	...
	private void Get_GiaVon(DataRowView odv)
	{
		DataTable dataTable = Commons.GetDataTable("asInGetGiaBQDD", CompanyInformations.CompanyID, RuntimeHelpers.GetObjectValue(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(odv["ma_vt"]), isNum: false)), RuntimeHelpers.GetObjectValue(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(odv["ma_kho"]), isNum: false)), RuntimeHelpers.GetObjectValue(txtNgay_Ct.Value));
		if ((dataTable != null && dataTable.Rows.Count > 0) ? true : false)
		{
			double num = Conversions.ToDouble(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(dataTable.Rows[0]["Gia_nt"]), isNum: true));
			double num2 = Conversions.ToDouble(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(dataTable.Rows[0]["Gia"]), isNum: true));
			odv["Gia"] = num2;
			if (Operators.CompareString(cboMa_NT.MA_NT, AsiaErp.Framework.Environment.GetSIConfiguration().ma_nt0, false) == 0)
			{
				odv["Gia_nt"] = num2;
	...
	private bool ValidTonKhoWhenSaveData()
	{
		if (CT1Voucher == null || CT1Voucher.Count <= 0)
		{
			return false;
		}
		bool result = true;
		...
		{
			double num = 0.0;
			num = Conversions.ToDouble(Helper.Repl_Null(RuntimeHelpers.GetObjectValue(CT1Voucher.ToTable().Compute("Sum(so_luong)", Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject((object)"Stt_Rec='", base.CurrentPHRow["Stt_Rec"]), (object)"' and ma_vt = '"), item["ma_vt"]), (object)"'")))), isNum: true));
			if (Operators.ConditionalCompareObjectGreater((object)num, Helper.Repl_Null(RuntimeHelpers.GetObjectValue(item["ton"]), isNum: true), false))
			{
				result = Commons.ProcessLackOfInventory2Issue();
				break;
			}
		}
		return result;
	...
	protected override void InitOtherBeforeLoadData()
	{
		...
		if (dmMagd != null && dmMagd.Count != 0 && 0 == 0)
		{
	...
	{
		if (base.CurrentPHRow != null && CTTable != null && 0 == 0)
		{
	...
	protected override bool ValidData()
	{
		...
		bool flag = true;
		bool flag2 = false;
		if ((ChkEmtyTextBox((TextBox)(object)txtMa_Kh) && base.ValidData() && ValidTonKhoWhenSaveData()) ? true : false)
		{
			bool flag3 = true;
			bool flag4 = true;
