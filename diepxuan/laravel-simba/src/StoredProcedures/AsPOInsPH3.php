<?php

declare(strict_types=1);

/*
 * @copyright  © 2019 Dxvn, Inc.
 *
 * @author     Tran Ngoc Duc <ductn@diepxuan.com>
 * @author     Tran Ngoc Duc <caothu91@gmail.com>
 *
 * @lastupdate 2026-02-12 12:20:00
 */

namespace Diepxuan\Simba\StoredProcedures;

use Diepxuan\Simba\SModel\SModel;
use Illuminate\Support\Collection;
use Diepxuan\Simba\Helper\ParamHelper;
/**
 * Class AsPOInsPH3
 *
 * Stored procedure: asPOInsPH3
 * Purpose:
 *
 * Parameters:
 * - @pMa_cty (nvarchar(3)): input parameter.
 * - @pStt_rec (nvarchar(20)): input parameter.
 * - @pMa_ct (nvarchar(3)): input parameter.
 * - @pMa_gd (nvarchar(1)): input parameter.
 * - @pMau_hd (nvarchar(1)): input parameter.
 * - @pMa_kh (nvarchar(20)): input parameter.
 * - @pDia_chi (nvarchar(255)): input parameter.
 * - @pNguoi_gd (nvarchar(30)): input parameter.
 * - @pMa_httt (nvarchar(20)): input parameter.
 * - @pTk_pt (nvarchar(20)): input parameter.
 * - @pDien_giai (nvarchar(255)): input parameter.
 * - @pGhi_chu (nvarchar(255)): input parameter.
 * - @pSo_seri (nvarchar(12)): input parameter.
 * - @pSo_ct (nvarchar(12)): input parameter.
 * - @pNgay_ct (smalldatetime): input parameter.
 * - @pNgay_lct (smalldatetime): input parameter.
 * - @pKht_tain (BIT): input parameter.
 * - @pNgay_pn (smalldatetime): input parameter.
 * - @pSo_pn (nvarchar(12)): input parameter.
 * - @pNgay_hd (smalldatetime): input parameter.
 * - @pSo_hd (nvarchar(12)): input parameter.
 * - @pMa_tt_po (nvarchar(8)): input parameter.
 * - @pHan_ck (decimal(19,4)): input parameter.
 * - @pTl_ck (decimal(19,4)): input parameter.
 * - @pHan_tt (decimal(19,4)): input parameter.
 * - @pLs_qh (decimal(19,4)): input parameter.
 * - @pMa_nt (nvarchar(3)): input parameter.
 * - @pTy_gia (decimal(19,4)): input parameter.
 * - @pT_tien_nt0 (decimal(19,4)): input parameter.
 * - @pT_tien0 (decimal(19,4)): input parameter.
 * - @pT_thue_nk_nt (decimal(19,4)): input parameter.
 * - @pT_thue_nk (decimal(19,4)): input parameter.
 * - @pT_thue_ttdb_nt (decimal(19,4)): input parameter.
 * - @pT_thue_ttdb (decimal(19,4)): input parameter.
 * - @pT_cp_nt (decimal(19,4)): input parameter.
 * - @pT_cp (decimal(19,4)): input parameter.
 * - @pT_tien_nt (decimal(19,4)): input parameter.
 * - @pT_tien (decimal(19,4)): input parameter.
 * - @pT_thue_nt (decimal(19,4)): input parameter.
 * - @pT_thue (decimal(19,4)): input parameter.
 * - @pTl_ck_hd (decimal(19,4)): input parameter.
 * - @pT_ck (decimal(19,4)): input parameter.
 * - @pT_ck_nt (decimal(19,4)): input parameter.
 * - @pT_tt (decimal(19,4)): input parameter.
 * - @pT_tt_nt (decimal(19,4)): input parameter.
 * - @pT_so_luong (decimal(19, 4)): input parameter.
 * - @pSua_tien (bit): input parameter.
 * - @pSua_thue (bit): input parameter.
 * - @pSua_thue_nk (bit): input parameter.
 * - @pSua_thue_ttdb (bit): input parameter.
 * - @pSua_tt (bit): input parameter.
 * - @pTao_tu_pn (bit): input parameter.
 * - @pTao_tu_dh (bit): input parameter.
 * - @pTs_nk (decimal (19,4)): input parameter.
 * - @pTs_ttdb (decimal (19,4)): input parameter.
 * - @pPost2gl (bit): input parameter.
 * - @pTrang_thai (nvarchar(1)): input parameter.
 * - @pLUser (nvarchar(20)): input parameter.
 * - @pRet (int): output parameter.
 *
 * Default values:
 *
 * Returns: Collection of query results.
 *
 * Example call:
 * ```php
 * $params = [
 *     // Provide parameter values
 * ];
 * $result = AsPOInsPH3::call($params);
 * ```
 */
class AsPOInsPH3
{
    public static function call(array $params): Collection
    {
        $paramObj = ParamHelper::fromArray($params);
        $connection = (new SModel())->getConnectionName();

        return ProcedureCaller::call('asPOInsPH3', [
            'pMa_cty' => $paramObj->pMa_cty ?? null,
            'pStt_rec' => $paramObj->pStt_rec ?? null,
            'pMa_ct' => $paramObj->pMa_ct ?? null,
            'pMa_gd' => $paramObj->pMa_gd ?? null,
            'pMau_hd' => $paramObj->pMau_hd ?? null,
            'pMa_kh' => $paramObj->pMa_kh ?? null,
            'pDia_chi' => $paramObj->pDia_chi ?? null,
            'pNguoi_gd' => $paramObj->pNguoi_gd ?? null,
            'pMa_httt' => $paramObj->pMa_httt ?? null,
            'pTk_pt' => $paramObj->pTk_pt ?? null,
            'pDien_giai' => $paramObj->pDien_giai ?? null,
            'pGhi_chu' => $paramObj->pGhi_chu ?? null,
            'pSo_seri' => $paramObj->pSo_seri ?? null,
            'pSo_ct' => $paramObj->pSo_ct ?? null,
            'pNgay_ct' => $paramObj->pNgay_ct ?? null,
            'pNgay_lct' => $paramObj->pNgay_lct ?? null,
            'pKht_tain' => $paramObj->pKht_tain ?? false,
            'pNgay_pn' => $paramObj->pNgay_pn ?? null,
            'pSo_pn' => $paramObj->pSo_pn ?? null,
            'pNgay_hd' => $paramObj->pNgay_hd ?? null,
            'pSo_hd' => $paramObj->pSo_hd ?? null,
            'pMa_tt_po' => $paramObj->pMa_tt_po ?? null,
            'pHan_ck' => $paramObj->pHan_ck ?? 0.0,
            'pTl_ck' => $paramObj->pTl_ck ?? 0.0,
            'pHan_tt' => $paramObj->pHan_tt ?? 0.0,
            'pLs_qh' => $paramObj->pLs_qh ?? 0.0,
            'pMa_nt' => $paramObj->pMa_nt ?? null,
            'pTy_gia' => $paramObj->pTy_gia ?? 0.0,
            'pT_tien_nt0' => $paramObj->pT_tien_nt0 ?? 0.0,
            'pT_tien0' => $paramObj->pT_tien0 ?? 0.0,
            'pT_thue_nk_nt' => $paramObj->pT_thue_nk_nt ?? 0.0,
            'pT_thue_nk' => $paramObj->pT_thue_nk ?? 0.0,
            'pT_thue_ttdb_nt' => $paramObj->pT_thue_ttdb_nt ?? 0.0,
            'pT_thue_ttdb' => $paramObj->pT_thue_ttdb ?? 0.0,
            'pT_cp_nt' => $paramObj->pT_cp_nt ?? 0.0,
            'pT_cp' => $paramObj->pT_cp ?? 0.0,
            'pT_tien_nt' => $paramObj->pT_tien_nt ?? 0.0,
            'pT_tien' => $paramObj->pT_tien ?? 0.0,
            'pT_thue_nt' => $paramObj->pT_thue_nt ?? 0.0,
            'pT_thue' => $paramObj->pT_thue ?? 0.0,
            'pTl_ck_hd' => $paramObj->pTl_ck_hd ?? 0.0,
            'pT_ck' => $paramObj->pT_ck ?? 0.0,
            'pT_ck_nt' => $paramObj->pT_ck_nt ?? 0.0,
            'pT_tt' => $paramObj->pT_tt ?? 0.0,
            'pT_tt_nt' => $paramObj->pT_tt_nt ?? 0.0,
            'pT_so_luong' => $paramObj->pT_so_luong ?? 0.0,
            'pSua_tien' => $paramObj->pSua_tien ?? false,
            'pSua_thue' => $paramObj->pSua_thue ?? false,
            'pSua_thue_nk' => $paramObj->pSua_thue_nk ?? false,
            'pSua_thue_ttdb' => $paramObj->pSua_thue_ttdb ?? false,
            'pSua_tt' => $paramObj->pSua_tt ?? false,
            'pTao_tu_pn' => $paramObj->pTao_tu_pn ?? false,
            'pTao_tu_dh' => $paramObj->pTao_tu_dh ?? false,
            'pTs_nk' => $paramObj->pTs_nk ?? 0.0,
            'pTs_ttdb' => $paramObj->pTs_ttdb ?? 0.0,
            'pPost2gl' => $paramObj->pPost2gl ?? false,
            'pTrang_thai' => $paramObj->pTrang_thai ?? null,
            'pLUser' => $paramObj->pLUser ?? null,
        ], $connection);
    }
}
