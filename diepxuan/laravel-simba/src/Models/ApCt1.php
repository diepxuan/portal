 ...
 * Class ApCt1.
 *
 * This class represents the model for accounts payable transactions.
 */
class ApCt1 extends Model
{
    ...
    {
        if (!empty($maNcc)) {
            return $query->where('ma_ncc', $maNcc);
        }
    ...
    public function apDmNcc()
    {
        return $this->belongsTo(ApDmNcc::class, 'ma_ncc', 'ma_ncc');
    }
    ...
    /**
     * Tính tổng tiền thanh toán theo nhà cung cấp
     */
    public static function getTotalPaymentBySupplier(string $maCty, ?string $fromDate = null, ?string $toDate = null): Collection
    {
        ...
        return $query->select('ma_ncc', DB::raw('SUM(tien_tt) as tong_tien_tt'), DB::raw('SUM(tien_tt_nt) as tong_tien_tt_nt'))
            ->groupBy('ma_ncc')
            ->get();
    }

    /**
     * Gọi stored procedure asAPRptCN01 để lấy dữ liệu báo cáo công nợ phải trả
     */
    public static function getAPRptCN01(array $params): Collection
    {
        return collect(DB::connection((new static())->getConnectionName())->select('EXEC asAPRptCN01
            @pMa_Cty = :pMa_Cty,
            @pNgay_Ct1 = :pNgay_Ct1,
            @pNgay_Ct2 = :pNgay_Ct2,
            @pMa_Ncc = :pMa_Ncc,
    ...
    /**
     * Gọi stored procedure asAPRptTM01 để lấy dữ liệu báo cáo thanh toán
     */
    public static function getAPRptTM01(array $params): Collection
    {
        return collect(DB::connection((new static())->getConnectionName())->select('EXEC asAPRptTM01
            @pMa_Cty = :pMa_Cty,
            @pNgay_Ct1 = :pNgay_Ct1,
            @pNgay_Ct2 = :pNgay_Ct2,
            @pMa_Ncc = :pMa_Ncc,
